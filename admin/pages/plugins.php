...
class page {
	...
	public function __construct() {
		$this->navtitle = "Plugins Sub Menu";
		$this->navlist[] = array("View plugins", "magnifier.png", "view");
		$this->navlist[] = array("Configure plugins", "magnifier.png", "configure");		
		...
		switch($main->getvar['sub']) {
			...
			case 'view':
				...
				if ($_POST) {
					...
					$new_list = new plugin_controller();
					$new_list->processPlugins($main->postvar);
				...
				$plugins_list 		= $plugin_controller->availablePlugins();				
				$plugin_name_list 	= $plugin_controller->get_plugin_name_type_list();
					
				foreach($plugins_list as $key => $plugins_list_type) {					
					$array['PLUGINS'] .= '<h2>'.$plugin_name_list[$key].'</h2>';
					$array['PLUGINS'] .= '<table width="100%" border="0" cellspacing="0" cellpadding="1">';
					if (!empty($plugins_list_type)) {
						foreach($plugins_list_type as $plugin) {
							$plugin['name'] = strtolower($plugin['name']);
							$array['PLUGINS'] .= '<tr>';
							$array['PLUGINS'] .= '<td width="34px"><input name="plugin_'.$plugin['name'].'" id="pages_'.$plugin['name'].'" type="checkbox"  /></td>';
							$array['PLUGINS'] .= '<td  align="left"><strong>'.$plugin['name'].'</strong><br />'.$plugin['description'].'<br />Author : '.$plugin['author'].'</td></tr>';	
						}
					}
					$array['PLUGINS'] .='</table>';						
				}
				...
				//global $type;
				//$array['FORM'] = $type->acpPedit(array(), array());						
				
				echo $style->replaceVar("tpl/editplugins.tpl", $array);
			...
			case 'configure':
				$plugin_controller  = new plugin_controller();	
				$plugin_list = $plugin_controller->installedPlugins();	
